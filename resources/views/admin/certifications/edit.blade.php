...
@section('page-title', 'Modify Certification')
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-4xl">
    <form action="{{ route('admin.certifications.update', $certification) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" required value="{{ old('title', $certification->title) }}"
        ...
        <div>
            ...
            <input type="text" name="description_heading" id="description_heading" value="{{ old('description_heading', $certification->description_heading) }}"
                   class="block w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none transition-all"
                   placeholder="e.g. Quality Management System">
        </div>

        <!-- Certification Badge / Logo -->
        <div x-data="{ preview: '{{ $certification->image ? asset($certification->image) : '' }}' }">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      ...
                      placeholder="Brief details about the audit standards, validation status, or description...">{{ old('description', $certification->description) }}</textarea>
        </div>
