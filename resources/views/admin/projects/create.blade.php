...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-4xl">
    <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
        @csrf
                    ...
                    <div class="relative z-10 w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden p-6 border border-slate-100" @click.away="modalOpen = false">
                        ...
                        <div class="flex items-center justify-between gap-3 mt-6 pt-4 border-t border-slate-100">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" @click="modalOpen = false" class="px-4 py-2 rounded-lg border border-slate-200 text-slate-600 font-semibold hover:bg-slate-50 text-xs transition-colors font-sans">
                                    Cancel
                                </button>
                                <button type="button" @click="submitNewSector()" :disabled="loading"
                                        class="px-4 py-2 rounded-lg bg-[#0f2343] hover:bg-primary text-white font-semibold text-xs flex items-center gap-1.5 shadow-sm hover:shadow-md transition-colors disabled:opacity-50 font-sans">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      ...
                      placeholder="Enter specific details about the project scope, duration, or outcomes...">{{ old('description') }}</textarea>
        </div>
